...
import { cloneDeep, cloneDeepWith, isEqual, merge, pick } from "lodash";
import { configure } from "safe-stable-stringify";
const stringify = configure({ deterministic: true });

export const startVersion = "2024-04";
...
export const inferSchemaFromExamplePayload = (
  ...
  for (const [matcher, override] of manualExamples) {
    if (matcher.test(metadata.name)) {
      ...
      const maskedOverride = pick(override, Object.keys(examplePayload));
      merge(overridesPayload, maskedOverride);
...
const unknownPaths: [topicPattern: RegExp, paths: string[]][] = [
  [
    /checkouts\/.+/,
    [
...
export const manualExamples: [RegExp, Record<string, any>][] = [
  [
    /.+/,
    {
